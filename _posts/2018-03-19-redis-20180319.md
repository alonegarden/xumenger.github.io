...
tags: Mac Linux Ubuntu c c++ 数据库 python redis nosql 持久化 内核 磁盘控制器 RDB 事件循环 阻塞 save bgsave fork Copy-On-Write vim AOF MySQL binlog AOF重写 主从 备份 slaveof 集群 sentinel 缓存 分布式缓存 分布式 lua 安全 ssh 公钥 秘钥 nginx Redis网络协议 协议 
---
...
本节来几个实际一点的例子更好的展示Redis下Lua的用法

### 记录短时间内频繁访问网页的IP

>[《Redis执行Lua脚本详细实例》](http://blog.csdn.net/tjcyjd/article/details/71636309?utm_source=itdadao&utm_medium=referral)
...
* incr key：将key中存储的数字值增一。如果key不存在，那么key的值会先被初始化为0，然后再执行incr操作
* expire key seconds：为key值设置生存时间，当key过期时，它会被自动删除，Redis内部通过定时器实现
...
>[《基于Redis Lua脚本实现的分布式锁》](http://ju.outofmemory.cn/entry/74765)
