...
tags: c c++ 数据库 python redis nosql 持久化 内核 磁盘控制器 RDB 阻塞 save bgsave fork Copy-On-Write vim AOF MySQL binlog AOF重写 主从 备份 sentinel 缓存 分布式缓存 分布式 lua 安全 nginx
---
...
然后执行save命令，这时候我们来到**redis-server**所运行的目录，可以看到有一个dump.rdb文件，然后我们可以使用vim（`vim -b dump.rbd`，然后`:%!xxd`查看其16进制格式）来查看该二进制文件（或者用`od -c dump.rdb`命令）

![image](../media/image/2018-03-19/01.png)

>参考[《Redis RDB格式》](https://www.jianshu.com/p/b1b34f77a3ac)了解详细的RDB文件格式！

Ubuntu下使用`sudo apt-get install redis-server`方式安装的Redis，其配置文件放在/etc/redis/redis.conf。在Mac下使用`brew install redis`方式安装的Redis，其配置文件位置为/usr/local/etc/redis.conf。当然也可以通过`redis-server config-path`的方式在redis-server启动时指定对应的配置文件路径
...
显然，如果每条写命令都生成一条日志，那么AOF文件会越变越大，所以Redis又提供了一个新功能，叫做AOF重写，其功能就是重新生成一份AOF文件，新的AOF文件中一条记录的操作只会有一次，而不像一份老文件那样可能记录了对同一值的多次写操作。其生成过程和RDB类似，也是fork一个进程，直接遍历数据，写入新的AOF临时文件。在写入新文件的过程中，所有的写操作日志还是会写到原来老的AOF文件中，同时还会记录在内存缓冲区中。当重完操作完成后，会将所有缓冲区中的日志一次性写入到临时文件中。然后调用原子性的rename命令用新的AOF文件取代老的AOF文件
