...
tags: Mac Linux Ubuntu c c++ 数据库 python redis nosql 持久化 内核 磁盘控制器 RDB 阻塞 save bgsave fork Copy-On-Write vim AOF MySQL binlog AOF重写 主从 备份 slaveof 集群 sentinel 缓存 分布式缓存 分布式 lua 安全 ssh 公钥 秘钥 nginx
---

>继[《一篇学会C++！》](http://www.xumenger.com/cpp-one-paper-20171230/)之后，又来了新的《一篇学会？？？》
...
* `ssh -i id_rsa xumenger@172.16.192.153` 通过id\_rsa秘钥文件以xumenger用户身份登录172.16.192.153主机
...
# 退出Redis客户端命令行，使用下面的命令设置一个新的键值对，将公钥信息写到值中
