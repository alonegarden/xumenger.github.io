...
>继[《一篇学会C++！》](http://www.xumenger.com/cpp-one-paper-20171230/)之后，又一个《一篇学会？？？》博客
...
* redis-sentinel sentinel-conf-path 或 redis-server sentinel-conf-path --sentinel 可以启动Sentinel
* ssh-keygen -t rsa -C "crack@redis.com" 制作SSH秘钥和公钥
* ssh -i id\_rsa xumenger@172.16.192.153 通过id\_rsa秘钥文件以xumenger用户登录到172.16.192.153这台主机
...
然后`ssh-keygen -t rsa -C "crack@redis.com"`制作SSH秘钥和公钥
