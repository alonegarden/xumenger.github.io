...
以下的内容是我查阅(转载)了大量的各种资料整理得到的。本文包含一些实践内容，这些实践操作依赖的基础数据表、数据，在文章的开始我先准备好！这里我选择最经典的博客系统的关系型数据表设计方案！
...
>【循环嵌套连接】的图表十分形象，出在上面的外部输入(Outer input)因为是post\_id这个非索引列，所以是表扫描。处于下面的内部输入(Inner input)因为id是主键，所以是聚集索引查找

外部输入仅执行一次，根据外部输入满足join条件的每一行，对内部输入进行查找。这里由于外部输入为7行，所以内部输入执行7次
