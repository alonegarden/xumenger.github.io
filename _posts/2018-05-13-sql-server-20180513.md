...
因为工作中经常和SQL Server打交道，而且最近做的一个基于SQL Server的项目中因为性能问题还专门针对SQL插入慢而进行了性能调优，所以想把自己对SQL Server的简单研究梳理成文
...
以下的内容主要是我查阅了大量的各种资料整理得到的。而且本文包含一些实践内容，这些实践操作依赖的基础数据表、数据，在文章的开始我先准备好！这里我选择最经典的博客系统的关系型数据表设计方案！
...
创建好数据表之后，在表中造一些用于后续测试的数据，测试数据量很少，但是在随便一个真实的生产环境数据量会大得多，因为只是演示，暂时就通过这些小数据来以小见大！
...
![image](../media/image/2018-05-13/10-01.png)
...
![image](../media/image/2018-05-13/11-02.png)
...
>在[《MySQL高级 之 explain执行计划详解》](https://blog.csdn.net/wuseyukui/article/details/71512793)中有对MySQL执行计划的详细讲解！
