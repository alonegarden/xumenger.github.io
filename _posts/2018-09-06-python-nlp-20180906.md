...
train.txt的内容大概是这样的。标签是以字符串label作为前缀的单词，后面的数字对应类别。我们可以将训练数据处理成这种形式，训练自己的文本分类模型（这里分别有别人整理好的[训练数据](http://pan.baidu.com/s/1jH7wyOY)和[测试数据](http://pan.baidu.com/s/1slGlPgx)）
