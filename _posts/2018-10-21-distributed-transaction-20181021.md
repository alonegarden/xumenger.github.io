...
title: TCC分布式事务
categories: 大型系统架构 
tags: 分布式事务 TCC TCC-Transaction 事务 数据库 ACID 数据一致性 注解 柔性事务 刚性事务 CAP BASE 单体应用 微服务 微服务架构 高可用性 消息队列 MQ Kafka 调用链 2PC 
---
...
本地消息表这种实现方式是业界使用最多的，其核心思想是**将分布式事务拆分成本地事务进行处理**
...
消息消费方，需要处理这个消息，并完成自己的业务逻辑。如果本地事务处理成功，表明已经处理成功了；如果处理失败，那么重试。如果是业务上面的失败，可以给生产方发送一个业务补偿消息，通知生产方进行回滚等操作

生产方和消费方定时扫描本地消息表，把还没处理完成的消息或者失败的消息再发送一遍。如果有靠谱的自动对账补账逻辑，这种方案还是非常实用的

这种方案遵循BASE 理论，采用的是最终一致性，该模式是几种方案里面比较适合实际业务场景的，既不会出现像2PC 那样复杂的实现(当**调用链**很长的时候，2PC 的可用性是非常低的)，也不会像TCC 那样可能出现确认或者回滚不了的情况
