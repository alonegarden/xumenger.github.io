...
```sql
select * from test_table where id = '1000' lock in share mode;
```
...
```sql
select * from test_table where id = '1000' for update;
```
...
```sql
insert into test_table(id, context) values('1001', 'sisiihsdidhs');
```
...
```sql
update test_table set context = '12345645' where id = '1001';
...
```java
try {
    insert('1001', 'insert message');
} catch (DuplicateKeyException e) {
    update('1001', 'update message');
}
```
...
```sql
INSERT INTO test_table (id, context) VALUES ('1001', '1223424s') ON DUPLICATE KEY UPDATE context = '1232ewsasd';
```
...
----|--------|-------|-----------|---------------|--------------|-------------|--------------|---------------------|
814 |2135    |3      |APPHIGH    |xumenger       |xumenger-OAPC |unknown      |test_table    | 2020-10-10 10:07:33 |
```
...
```sql
insert into test_table(id, context) values('1002', 'sisiihsdidhs');
```
...
SID |SERIAL# |锁模式 |登录用户   |登录机器用户名 |机器名        |终端用户名   |被锁对象名    |登录数据库时间       |
----|--------|-------|-----------|---------------|--------------|-------------|--------------|---------------------|
814 |2135    |3      |APPHIGH    |xumenger       |xumenger-OAPC |unknown      |test_table    | 2020-10-10 14:07:33 |
```
