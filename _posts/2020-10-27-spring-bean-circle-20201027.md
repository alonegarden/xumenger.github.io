...
tags: java bean IoC 循环依赖 Spring 容器 constructor-arg 设计模式 模板模式 BeanDefinition 依赖注入 singletonsCurrentlyInCreation inCreationCheckExclusions 动态代理 
---
...
* 因为ClassA 设置为non-lasy-init，所以在finishBeanFactoryInitialization() 时会调用getBean(ClassA) 对其进行依赖注入
...
* 创建ClassA 的过程中，因为其有constructor-arg 配置，所以会执行到resolveConstructorArguments()
    * 详细看上面的调用栈
    * 在这里看到其依赖ClassB
    * 然后会调用getBean(ClassB) 去对ClassB 进行依赖注入
* 先调用beforeSingletonCreation(ClassB); 将其添加到singletonsCurrentlyInCreation 中
* 在创建ClassB 的过程中，因为其有constructor-arg 配置，所以会执行到resolveConstructorArguments()
    * 在这里看到其依赖ClassA
