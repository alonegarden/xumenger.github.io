...
```java
BoolQueryBuilder wBoolQueryBuilder = QueryBuilders.boolQuery();

// 指定接口名
wBoolQueryBuilder.must(QueryBuilders.matchQuery("RequestHead.ApiName", "trade_query"));
...
wBoolQueryBuilder.must(QueryBuilders.rangeQuery("RequestHead.ReqTime").from(formatter.format(begin)).to(formatter.format(end)));
```
