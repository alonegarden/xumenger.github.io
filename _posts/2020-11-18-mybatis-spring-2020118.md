...
src/main/resources 下新增application.yml 配置文件
...
src/main/resources/demo11 下新增TestTableMapper 配置文件
...
public class TestController 
{
    ...
    public String user(@PathVariable("id") Integer id) {
		/// 通过在RoleMapper.xml 中<cache> 标签开启缓存，如果没有则每次都去数据库
...
上面的文章中通过自己创建SqlSession 来通过MyBatis 与数据库交互，在Spring 中使用MyBatis 完全没有了这个逻辑，但是显然与还是要有SqlSession 的，那这个是在哪里创建的
...
mybatis-spring-boot-autoconfigure-1.3.2.jar、mybatis-spring-1.3.2.jar、mybatis-3.4.6.jar 这三个是和spring、mybatis、自动配置相关的jar 包，可以好好看这里的代码
spring-boot-starter-web-1.5.6.RELEASE.jar、spring-boot-starter-tomcat-1.5.6.RELEASE.jar、spring-web-4.3.10.RELEASE.jar、spring-webmvc-4.3.10.RELEASE.jar 这些是和Web 相关的
